...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    if(white_detected)
    {
      ...
      if(pixel_column < 350) // white detected in left section
      {
        //ROS_INFO("Left");
        ang_z = 0.5 - 0.5 * (pixel_column / 350);
        lin_x = 0.5 * (pixel_column / 350);
        drive_robot(lin_x, ang_z);
      }

      else if(pixel_column < 350 && pixel_column < 450) // white detected in middle section
      {
        //ROS_INFO("Straight");
        ang_z = 0;
        lin_x = 0.5;
      ...
      {
        //ROS_INFO("Right");
        ang_z = -0.5 * ((pixel_column - 450) / 800 - 450);
        lin_x = 0.5 - ((pixel_column - 450) / 800 - 450);
        drive_robot(lin_x, ang_z);
      }
    }
